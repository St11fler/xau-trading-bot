...
# Configure logging
logging.basicConfig(
    level=logging.INFO,  # For more detailed logs, use logging.DEBUG
...
ATR_MULTIPLIER_SCALING = CONFIG.get("ATR_MULTIPLIER_SCALING", CONFIG["ATR_MULTIPLIER_SCALPING"])
ATR_MULTIPLIER_SCALPING = ATR_MULTIPLIER_SCALING
...
def load_models():
    ...
    try:
        ...
        logging.info("Scalping feature scaler loaded successfully.")
    except Exception as e:
        logging.error(f"Failed to load scalping feature scaler: {e}")
        sys.exit()

    try:
        target_scalers_scalping = joblib.load('scalping_target_scalers.pkl')
        logging.info("Scalping target scalers loaded successfully.")
    except Exception as e:
        logging.error(f"Failed to load scalping target scalers: {e}")
        sys.exit()
    ...
    try:
        ...
        logging.info("Long-term feature scaler loaded successfully.")
    except Exception as e:
        logging.error(f"Failed to load long-term feature scaler: {e}")
        sys.exit()

    try:
        target_scalers_longterm = joblib.load('longterm_target_scalers.pkl')
        logging.info("Long-term target scalers loaded successfully.")
    except Exception as e:
        logging.error(f"Failed to load long-term target scalers: {e}")
        sys.exit()

    try:
        clf, action_features = joblib.load('action_classifier.pkl')
        logging.info("Action classifier loaded successfully.")
    except Exception as e:
...
def execute_trade(signal, predicted_price, latest_close_price, data_merged, trade_timeframe, magic_number, lot_size, atr_multiplier):
    """Execute a scalping trade based on the generated signal."""
    try:
        # We handle both 'buy' and 'sell' as scalping trades now.
        ...
        if USE_ATR_SL_TP:
            # Dynamic SL and TP based on ATR and model predictions for scalping
            if signal == 'buy':
                sl = price - (atr_multiplier * atr_value)
                tp = predicted_price + (sl_tp_multiplier * atr_multiplier * atr_value)
                if tp <= price:
                    logging.warning("Predicted TP is not above current price for buy order.")
                    return
                if sl >= price:
                    logging.warning("Calculated SL is not below current price for buy order.")
                    return
            else:  # sell signal
                sl = price + (atr_multiplier * atr_value)
                tp = predicted_price - (sl_tp_multiplier * atr_multiplier * atr_value)
                if tp >= price:
                    logging.warning("Predicted TP is not below current price for sell order.")
                    return
                if sl <= price:
                    logging.warning("Calculated SL is not above current price for sell order.")
                    return
        else:
            # Fixed SL and TP (legacy fallback) for scalping trades
            if signal == 'buy':
                sl = price - FIXED_SL_SCALPING - (sl_tp_multiplier * mae)
                tp = price + FIXED_TP_SCALPING + (sl_tp_multiplier * mae)
                if tp <= price:
                    logging.warning("Fixed TP is not above current price for buy order.")
                    return
                if sl >= price:
                    logging.warning("Fixed SL is not below current price for buy order.")
                    return
            else:  # sell signal
                sl = price + FIXED_SL_SCALPING + (sl_tp_multiplier * mae)
                tp = price - FIXED_TP_SCALPING - (sl_tp_multiplier * mae)
                if tp >= price:
                    logging.warning("Fixed TP is not below current price for sell order.")
                    return
                if sl <= price:
                    logging.warning("Fixed SL is not above current price for sell order.")
                    return

        sl_distance_points = abs(price - sl) / point
        tp_distance_points = abs(tp - price) / point

        # Ensure SL and TP meet the minimum stop distance
        if sl_distance_points < stop_level:
            if order_type == mt5.ORDER_TYPE_BUY:
                sl = price - (stop_level * point)
            else:
                sl = price + (stop_level * point)
        ...
        try:
            ...
            logging.error(f"Error rounding SL/TP/Price: {e}")
            return
...
def main():
    ...
    while True:
        try:
            ...
            # Determine expected features from scalers
            ...
            # Add predicted prices to the last row of data_merged for classifier
            ...
            # Ensure all action_features exist
            ...
            action = clf.predict(X_action)[0]  # 1 or -1
            if action == 1:
                signal = 'buy'
            elif action == -1:
                signal = 'sell'
            ...
            if signal in ['buy', 'sell']:
                # Execute scalping trade for both buy and sell signals
                magic_number = MAGIC_NUMBER_SCALPING
                lot_size = LOT_SIZE_SCALPING
                atr_multiplier = ATR_MULTIPLIER_SCALPING
                trade_timeframe = 'M1'
                execute_trade(
                    signal=signal,
